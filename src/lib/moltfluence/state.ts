...
function resolveStatePath(): string {
  ...
  if (configured && configured.length > 0) {
    if (process.env.VERCEL && !configured.startsWith("/tmp/")) {
      const basename = configured.split("/").filter(Boolean).pop() || "moltfluence-state.json";
      return `/tmp/${basename}`;
    }
    return configured;
  }

  return process.env.VERCEL ? "/tmp/moltfluence-state.json" : join(process.cwd(), ".data", "moltfluence-state.json");
}
