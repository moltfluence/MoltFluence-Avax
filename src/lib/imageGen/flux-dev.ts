...
export const fluxDevAdapter: ImageGenAdapter = {
  model: "flux-dev",
  ...
  async generateImage(request: ImageGenRequest, apiKey: string): Promise<string> {
    ...
    if (!res.ok) {
      ...
      throw new Error(`Flux Dev API error ${res.status}: ${body}`);
    }

    const json: any = await res.json();
    const jobId = json.data?.task_id;
    if (!jobId) throw new Error("Flux Dev API did not return a task ID");
    return String(jobId);
  },
  ...
  async checkStatus(jobId: string, apiKey: string): Promise<ImageGenStatus> {
    ...
    if (!res.ok) {
      const body = await res.text().catch(() => "");
      throw new Error(`Flux Dev status error ${res.status}: ${body}`);
    }
